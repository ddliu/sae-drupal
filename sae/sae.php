...
function drupal_http_request($url, array $options = array()) {
  ...
  if($options['method'] == 'GET'){
	  $url = $url.((false === strpos($url, '?'))?'?':'&').$options['data'];
  }
  ...
  if($options['timeout']){
	  curl_setopt($curl, CURL_TIMEOUT, $options['timeout']);
  }
  /*
  ...
  if (!isset($responses[$curl_http_code])) {
    $curl_http_code = floor($curl_http_code / 100) * 100;
  }
